...
import net.codjo.test.release.task.gui.AssertTooltipStep;
...
public class AssertTooltipStepTest extends JFCTestCase {
    ...
    public void test_assertKOBadTooltip() throws Exception {
        ...
        try {
            step.proceed(context);
            fail("Pas d'erreur alors que l'expected ne correspond pas � l'actual !");
        }
        catch (Exception e) {
    ...
    private class TableForToolTipTest extends JTable {
        ...
        private final String[][] donnees = new String[][]{
              ...
              {"Gin", "Oasis", "standard"},
              {"GinLight", "boomerang", "comp�tition"},
              {"Advance", "Omega", "performance"}
        };

        private final String[] colonnes = {"marque", "mod�le", "homologation"};
